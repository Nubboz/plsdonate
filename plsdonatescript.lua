...
queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Nubboz/plsdonate/main/plsdonatescript.lua'))()")
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Nubboz/plsdonate/main/UIengine.lua"))()
...
local function update()
    local text
	local halfcurrent = Players.LocalPlayer.leaderstats.Raised.Value / 2
    local current = Players.LocalPlayer.leaderstats.Raised.Value
    local goal = current + tonumber(getgenv().settings.goalBox)
    if halfcurrent == 420 or halfcurrent == 425 then
        halfcurrent = halfcurrent + 10
    end
	...
	if halfcurrent > 999 then
        halfcurrent = string.format("%.2fk", current / 10 ^ 3)
    end
    if getgenv().settings.textUpdateToggle and getgenv().settings.customBoothText then
		...
		text = string.gsub (text, "$H", halfcurrent)
...
local helpLabel = boothTab:AddLabel("$C = Current, $G = Goal, 221 Character Limit $H 1/2 of current")
...
signHexBox.Text = getgenv().settings.signHexBox
    ...
    local signHelpLabel = signTab:AddLabel("$C = Current, $G = Goal, 221 Character Limit $H 1/2 of current")
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Loading..." then
    game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "PLS DONATE - Nubb"
end
